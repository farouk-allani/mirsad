...
- **`aave-v3/supply` takes base units; `web3/approve-token` takes human units.** Two adjacent actions in the same flow, opposite conventions, and the MCP schema says `string` for both. The unit is stated in the field label — `Amount (wei)` — and the schema builder drops the label; 102 protocol inputs across 14 protocol files lose their unit the same way. Filed as [KeeperHub/keeperhub#2466](https://github.com/KeeperHub/keeperhub/issues/2466), accepted by a maintainer the same night, fixed in [PR #2475](https://github.com/KeeperHub/keeperhub/pull/2475), approved twice: one expression in the shared builder plus a sweep test over every parenthesised label in the catalog, 321 fields. Our own Phase 0 notes guessed the other way and would have supplied 0.000001 USDC.
